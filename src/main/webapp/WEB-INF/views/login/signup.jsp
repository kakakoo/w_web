      ...
      <form id="loginForm" method="post">
          <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type="email" class="form-control" id="emailAddress" required placeholder="이메일 주소를 입력하세요">
                </div>
		  		<div class="form-group">
                  <label for="loginPassword">비밀번호</label>
                  <input type="password" class="form-control" id="newPwd" required placeholder="비밀번호를 입력하세요">
                </div>
		 		<div class="form-group">
                  <label for="loginPassword2">비밀번호 확인</label>
                  <input type="password" class="form-control" id="newPwdChk" required placeholder="비밀번호를 한번 더 입력하세요">
                </div>
		  		<div class="form-group">
                  <label for="frdCd">추천인번호</label>
                  <input type="password" class="form-control" id="frdCd" required placeholder="추천인번호를 입력하세요">
                </div>
...
개인정보처리방침에 대하여 내용 추가, 삭제 및 수정이 있을 시에는 개정 최소 7일전부터 ‘공지사항’을 통해 고지합니다. 다만, 개인정보의 수집 및 활용, 제3자 제공 등과 같이 회원 권리의 중요한 변경이 있을 경우에는 최소 30일 전에 고지합니다.</textarea></div>
					...
					<div class="form-check custom-control custom-checkbox" style="margin-top:20px;">
					  <input id="agree" name="remember" class="custom-control-input" type="checkbox">
					  <label class="custom-control-label" for="agree">이용약관에 동의합니다.</label>
				  </div>
                </div>
                
                <button class="btn btn-primary btn-block my-4 btn-noactive" id="btnSign" onclick="signup()" type="button">회원가입</button>
              </form>
	...
	$.ajax({
		...
		success : function(result) {
			...
			if (res == 'fail') {
				$('#usrTel').val('');
				$('#usrTel').addClass('error-form');
				$('#usrTel').attr('placeholder', '이미 가입된 번호입니다');
			} else {
...
function sendCode() {
	...
	if(cerfity_code == certi){
		...
		$('#m_desc').html('인증이 완료되었습니다.<br>정보를 입력해주세요.');
		$('#md_trig').trigger('click');
		$('#certy_grp').css('display','none');
		$('#btnSign').removeClass('btn-noactive');
...
function signup(){
	...
	if(checkFlag == 'N'){
		$('#m_title').html('회원가입 실패');
		$('#m_desc').html('휴대전화인증을 진행하지 않았습니다.<br>인증을 진행해주세요.');
		$('#md_trig').trigger('click');
		return;
	}
	...
	if (newPwd == '' || newPwdChk == '') {
		$('#newPwd').val('');
		$('#newPwdChk').val('');
		$('#newPwd').addClass('error-form');
		$('#newPwd').attr('placeholder', '비밀번호를 입력해주세요.');
		return;
	}
	...
	if(newPwd.length < 8){
		...
		$('#newPwdChk').val('');
		$('#newPwd').addClass('error-form');
		$('#newPwd').attr('placeholder', '비밀번호는 8자 이상으로 입력해주세요.');
		return;
	}
	...
	if (newPwd != newPwdChk) {
		...
		$('#newPwdChk').val('');
		$('#newPwd').addClass('error-form');
		$('#newPwd').attr('placeholder', '비밀번호가 일치하지 않습니다.');
		return;
	}
	
	var agree = $("input:checkbox[id='agree']").is(":checked");
	if(!agree){
		$('#m_title').html('이용약관 동의');
		$('#m_desc').html('이용약관에 동의를 해 주시기 바랍니다.');
		$('#md_trig').trigger('click');
		return;
	}
	...
	data["usrTel"] = tel.replace(/-/gi, '');
	data["usrNation"] = $('#usrNation').val();
	data["usrPw"] = $('#newPwd').val();
	data["frdCd"] = $('#frdCd').val();
	...
	$.ajax({
		...
		success : function(result) {
			...
			if (res == 'fail') {
				...
				alert('회원가입에 성공하였습니다.');
				location.href="${pageContext.request.contextPath}/signupDone";
			}
		},
		beforeSend : function() {
			$('.wrap-loading').removeClass('display-none');
