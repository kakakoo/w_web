							  ...
							  <div class="text-4 font-weight-600 my-4" style="color: #2a333d;">■ 예약자 정보를 알려주세요</div>
								<ul class="list-unstyled">
								  <div class="form-group">
									...
									<input type="text" class="form-control" id="usrNm" required placeholder="성명을 입력하세요">
								  </div>
								  <div class="form-group">
									<label for="newPassword">주민등록번호</label>
									<input type="tel" class="form-control" id="usrNmId" required placeholder="주민등록번호를 입력하세요('-'제외)">
								  </div>
								  <div class="form-group">
									<label for="newPassword">휴대전화번호</label>
									<input type="tel" class="form-control" id="usrTel" required placeholder="휴대전화번호를 입력하세요">
								  </div>
								  <div class="form-group">
									<label for="newPassword">이메일 주소</label>
									<input type="email" class="form-control" id="usrEmail" required placeholder="이메일 주소를 입력하세요">
								  </div>
							  </ul>
							  <p style="height: 10px;"></p>
...
<form name="formBody" action="${pageContext.request.contextPath}/p/guest" method="post">
...
$(function(){
	...
	if(ses == 'N'){
		alert('메인 화면으로 이동합니다.');
		location.href='${pageContext.request.contextPath}/p/${info.copCd}';
	}
		...
		$.ajax({
			...
			success : function(result) {
				...
				if(suc == 'success'){
					...
					FORM_BODY.attr('action', '${pageContext.request.contextPath}/p/guest');
					FORM_BODY.submit();
